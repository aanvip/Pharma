...
function deriveSourceType(i: Pick<Inquiry, 'source_type' | 'supplier_name'>): 'india' | 'china' | 'local' | null {
  const source = (i.source_type || '').toLowerCase();
  if (source === 'india' || source === 'china' || source === 'local') return source;
  const supplier = (i.supplier_name || '').toLowerCase();
  if (supplier.includes('china')) return 'china';
  if (supplier.includes('local')) return 'local';
  if (supplier.includes('india')) return 'india';
  return null;
}
...
export function SourcingOutbox() {
  ...
  const saveSourceDetails = async () => {
    ...
    const route = deriveSourceType(sourceModalRow) || 'india';
  ...
  const markSelectedForKunal = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        kunal_pricing_note: 'Sent to Kunal Pricing from Sourcing Outbox',
        updated_at: now,
      })
      .in('id', ids);
    ...
    showToast({ type: 'success', title: 'Sent to Kunal Pricing', message: `${rows.length} inquiry row${rows.length !== 1 ? 's' : ''} routed.` });
    await load();
  };
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-gray-500">Completed rows and local rows are skipped. India and China will send separately.</p>
                </div>
